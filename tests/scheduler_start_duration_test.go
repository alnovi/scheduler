...
func TestSchedulerDurationStart(t *testing.T) {
	testCases := []struct {
		name           string
		sleep          time.Duration
		...
		{
			name:         "Success duration WithEnabled",
			sleep:        5 * time.Minute,
			taskDuration: time.Minute,
			taskOpts: []tasks.Option{
		...
		{
			name:         "Success duration WithDelay",
			sleep:        5 * time.Minute,
			taskDuration: time.Minute,
			taskOpts: []tasks.Option{
		...
		{
			name:         "Success duration WithTimeout 1",
			sleep:        5 * time.Minute,
			taskDuration: time.Minute,
			taskOpts: []tasks.Option{
		...
		{
			name:         "Success duration WithTimeout 2",
			sleep:        5 * time.Minute,
			taskDuration: time.Minute,
			taskOpts: []tasks.Option{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				...
				err := schedule.AddTask("test-task", tasks.MustDurationTask(tc.taskDuration, handler, tc.taskOpts...))
				require.NoError(t, err, "failed to add task default")

				err = schedule.Start(ctx)
				require.NoError(t, err, "failed to start scheduler")
				err = schedule.Start(ctx)
				require.NoError(t, err, "failed to double start scheduler")
